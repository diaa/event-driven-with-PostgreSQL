...
docker exec -i "${POSTGRES_CONTAINER}" psql -U "${PGUSER}" -d "${PGDATABASE}" -c "SELECT pg_drop_replication_slot(slot_name) FROM pg_replication_slots WHERE slot_name IN ('wal2json_slot','debezium_slot','drasi_slot') AND NOT active;"
